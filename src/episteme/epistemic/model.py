...
class Prediction:
    ...
    Attributes:
        ...
        analyses: IDs of analyses whose results bear on this prediction.
            Existence-validated on registration and update. The reverse
            navigational link is ``Analysis.hypotheses_covered`` (via the
            shared hypothesis chain); there is no automatic backlink on
            ``Analysis`` itself.
...
@dataclass
class Experiment:
    """A physical experiment designed to generate empirical evidence.

    Experiments are the physical-world counterpart to ``Analysis`` entities.
    Where ``Analysis`` represents deterministic computational work
    (reproducible by checking out a git SHA and re-running a script),
    an ``Experiment`` is a physical event: it produces new ``Observation``
    entities and cannot be identically re-run.

    The lifecycle moves from ``PLANNED`` (intent recorded) through
    ``RUNNING`` to ``COMPLETE`` or ``ABANDONED``. Registering a
    ``PLANNED`` experiment lets AI agents and researchers express intent
    before execution, enabling queries like "what experiments are planned
    to test H-007?"

    ``observations`` is a backlink maintained automatically when
    ``Observation`` entities are registered with this experiment's ID.
    Callers should not set it directly.

    ``predictions_tested`` and ``assumptions_tested`` are structural
    forward links declaring design intent. They are existence-validated
    on registration but carry no automatic backlink — the same pattern
    as ``Prediction.hypothesis_ids``.

    Attributes:
        id: Unique identifier (e.g. ``"EXP-001"``).
        title: Human-readable name for this experiment.
        status: Lifecycle state. ``PLANNED``, ``RUNNING``, ``COMPLETE``,
            or ``ABANDONED``.
        protocol: What is being done and how to replicate it. Describes
            equipment, procedure, and conditions in enough detail that
            another researcher could reproduce the run.
        predictions_tested: IDs of predictions this experiment was
            designed to test. Structural forward link; existence-validated.
        assumptions_tested: IDs of assumptions this experiment was
            designed to probe. Structural forward link; existence-validated.
        observations: IDs of observations generated by this experiment.
            Backlink maintained by observation registration. Not set by
            callers.
        instrument: The primary instrument, apparatus, or tool used.
        conditions: Environmental or experimental conditions (temperature,
            pressure, reagent concentrations, etc.).
        date: Date the experiment was run or completed.
        replicate_of: ID of the experiment this run replicates. When set,
            the referenced experiment must already exist in the graph at
            registration time. ``None`` for original (non-replicate) runs.
        source: Lab notebook reference, DOI, protocol document URI, or
            any other primary provenance pointer.
        notes: Free-form notes for the researcher.
        tags: Free-form labels for filtering, grouping, or cross-cutting
            concerns.
    """
    ...
    protocol: str | None = None             # what was done / how to replicate
    predictions_tested: set[PredictionId] = field(default_factory=set)
    assumptions_tested: set[AssumptionId] = field(default_factory=set)
    observations: set[ObservationId] = field(default_factory=set)  # backlink
    replicate_of: ExperimentId | None = None  # ID of the experiment this replicates
    instrument: str | None = None
    conditions: str | None = None           # environmental / experimental conditions
    date: date | None = None
    source: str | None = None               # lab notebook ref, DOI, protocol doc
    notes: str | None = None
    tags: set[str] = field(default_factory=set)
...
class Observation:
    ...
    Attributes:
        ...
        related_assumptions: IDs of assumptions this observation is
            ...
            removal.
        experiment: ID of the experiment that produced this observation.
            Structural link to the parent ``Experiment``. Bidirectional with
            ``Experiment.observations``. ``None`` for observations not
            produced by a registered experiment (e.g. literature values,
            exploratory data).
    ...
    experiment: ExperimentId | None = None  # experiment that produced this observation
